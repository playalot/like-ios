...
@implementation LCJSONUtility

+ (id)objectFromJSONString:(NSString *)jsonStr {
    if (jsonStr == nil || jsonStr.length == 0) return nil;
    if (![jsonStr isKindOfClass:[NSString class]]) return nil;
    NSError *jsonParsingError = nil;
    id object = [NSJSONSerialization JSONObjectWithData:[LCJSONUtility dataFromJSONString:jsonStr] options:0 error:&jsonParsingError];
    return object;
}
...
+ (NSString *)stringFromJSONObject:(id)object {
    ...
    return [LCJSONUtility stringFromJSONData:data];
}
