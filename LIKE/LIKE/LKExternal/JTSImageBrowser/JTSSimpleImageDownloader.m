...
+ (NSURLSessionDataTask *)downloadImageForURL:(NSURL *)imageURL canonicalURL:(NSURL *)canonicalURL completion:(void (^)(UIImage *))completion {
    ...
    if (imageURL.absoluteString.length) {
        ...
        if (request == nil) {
            ...
            if ([LCUIImageCache.singleton hasCachedWithKey:imageURL.absoluteString]) {
                
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    
                    UIImage * image = [LCUIImageCache.singleton imageWithKey:imageURL.absoluteString];
            ...
            dataTask = [sesh dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                ...
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    ...
                    [LCUIImageCache.singleton saveImageToMemoryCache:image key:imageURL.absoluteString];
                    [LCUIImageCache.singleton saveImageDataToFileCache:data key:imageURL.absoluteString];
